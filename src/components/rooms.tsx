  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="font-serif text-2xl text-anthracite mb-2">{t(`room.${room.id}.name`)}</h3>
        <p className="text-gray-600 font-light text-sm leading-relaxed mb-4 flex-grow">{t(`room.${room.id}.desc`)}</p>
        <div className="flex items-center text-xs uppercase tracking-widest text-gold font-medium mt-auto">
          {t("rooms.viewDetails")} <ChevronRight className="w-3 h-3 ml-1" />
        </div>
      </div>
    </article>
  );
...
function RoomModal({
  ...
  const whatsappUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
    `${t("rooms.waMessage")} "${t(`room.${room.id}.name`)}" (${t(`room.${room.id}.priceNote`)}). ${t("rooms.waAvailability")}`
  )}`;
      ...
      >
        ...
        <div className="w-full md:w-2/5 flex flex-col h-full overflow-y-auto bg-white">
          <div className="p-8 md:p-12">
            <h2 className="font-serif text-3xl md:text-4xl text-anthracite mb-1">{t(`room.${room.id}.name`)}</h2>
            <div className="flex items-baseline space-x-2 mb-4">
              <span className="text-2xl font-serif text-gold">${room.price}</span>
              <span className="text-sm text-gray-500 font-light">{t("rooms.perNight")}</span>
            </div>
            <div className="w-12 h-0.5 bg-gold mb-8"></div>
            <p className="text-gray-600 font-light leading-relaxed mb-8 text-lg">{t(`room.${room.id}.long`)}</p>

            <h4 className="font-serif text-xl text-anthracite mb-6">{t("rooms.amenities")}</h4>
            ...
            {room.hasExtraNote && (
              <div className="flex items-start space-x-3 bg-gold/5 border border-gold/20 rounded-lg p-4 mb-8">
                ...
                <p className="text-sm text-gold/80 leading-relaxed">{t(`room.${room.id}.extraNote`)}</p>
              </div>
            )}
            ...
            <div className="mt-auto pt-6 border-t border-gray-100 space-y-3">
              <a href="#reservas" onClick={onClose} className="flex items-center justify-center w-full py-3 border border-anthracite text-anthracite hover:bg-anthracite hover:text-white font-medium transition-colors text-sm uppercase tracking-widest gap-2">
                <CalendarDays className="w-4 h-4" /> {t("rooms.checkAvailability")}
              </a>
              <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full py-4 bg-[#25D366] hover:bg-[#20bd5a] text-white font-medium transition-colors rounded shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 duration-200">
                <MessageCircle className="w-5 h-5 mr-2" /> {t("rooms.bookWhatsapp")}
              </a>
              <p className="text-center text-xs text-gray-400">{t("rooms.priceDisclaimer")}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="habitaciones" className="py-24 px-6 bg-[#F9F7F2]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="font-serif text-4xl text-anthracite mb-4">{t("rooms.title")}</h2>
